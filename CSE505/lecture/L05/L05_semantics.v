...
(** "Big Step" holds for the final result. *)
(** not: eval h (1 [+] 2 [+] 3) (1 [+] 5) *)
...
Definition ellis_stuck (h: heap) (s: stmt) : Prop :=
  forall h' s',
  ~ step h s h' s'.
...
(**
Fixpoint run (h: heap) (s: stmt) : (heap * stmt) :=
...
(** NOTE: we would have a much harder time
          if we had defined step_n to
          "add on the right":
*)
...
(**
  Try using this definition and doing the above proofs.
*)
